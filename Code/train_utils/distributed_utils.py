...
class ConfusionMatrix(object):
    ...
    def __str__(self):
        ...
        return (
            ...
            metrics["acc_global"].item() * 100,
            ['{:.1f}%'.format(x * 100) for x in metrics["acc"].tolist()],
            ['{:.1f}%'.format(x * 100) for x in metrics["precision"].tolist()],
            ['{:.1f}%'.format(x * 100) for x in metrics["recall"].tolist()],
            ['{:.1f}%'.format(x * 100) for x in metrics["f1"].tolist()],
            ['{:.1f}%'.format(x * 100) for x in metrics["iu"].tolist()],
            metrics["iu"].mean().item() * 100,
            metrics["precision"].mean().item() * 100,
            metrics["recall"].mean().item() * 100,
            metrics["f1"].mean().item() * 100
        )
